...
public class SCAdminCheckinTicketViewScreen extends ListActivity {
	...
	public void onClickCheckIn(View v) {
		...
		AssetsTicketsInfo tInfo = (AssetsTicketsInfo) listView
				.getItemAtPosition(position);
		Log.v("tickect info", "tickect info" + tInfo + "position \t" + position);
		Intent checkIn = new Intent(mContext, ScAdminCheckInScreen.class);
		checkIn.putExtra("position", position);
	...
	private class AssetTicketTask extends AsyncTask<String, Integer, Boolean> {
						...
						if (!key.equalsIgnoreCase("route_order")
							...
							if (ticketArray != null && ticketArray.length() > 0) {
								for (int i = 0; i < ticketArray.length(); i++) {
									...
									Log.v("ticket_type","ticket_type"+ticketObj
											.getString("ticket_type"));
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pdialog = new ProgressDialog(SCAdminCheckinTicketViewScreen.this);
