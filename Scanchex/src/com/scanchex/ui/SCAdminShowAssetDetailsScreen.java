...
public class SCAdminShowAssetDetailsScreen extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		RelativeLayout layout = (RelativeLayout) findViewById(R.id.adminAssetScreen);
		layout.setBackgroundColor(SCPreferences
				.getColor(SCAdminShowAssetDetailsScreen.this));
		mContext = this;
		SCAdminAssetDetailsInfo assetInfo = Resources.getResources()
				.getAssetDetailInfo();
		...
		ImageView imageView = (ImageView) findViewById(R.id.asset_image_id);

		try {
//			Picasso.with(mContext).load(assetInfo.assetImage);
//					.placeholder(R.drawable.photo_not_available)
//					.error(R.drawable.app_icon).into(imageView);
			
			imageView.setImageBitmap(assetInfo.assetImage);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		assetAddress.setText(assetInfo.street + "\n" + assetInfo.city + ", "
				+ assetInfo.state + " " + assetInfo.postalCode);
	...
	protected void onRestart() {
		...
		if (selectedImagePath != null && selectedImagePath.length() > 0) {
			takePhotoBtn.setText("Submit");
			assetImage.setImageBitmap(BitmapFactory.decodeFile(selectedImagePath));
		}
	}
	...
	private void showAlertDialog(String title, String message) {
		new AlertDialog.Builder(SCAdminShowAssetDetailsScreen.this)
				.setIcon(R.drawable.info_icon).setTitle(title)
				.setMessage(message)
	...
	private class UpdateAdminLocationTask extends
		...
		protected Boolean doInBackground(String... params) {
			try {
				...
				listParams.add(new BasicNameValuePair("master_key", ""
						+ SCPreferences.getPreferences().getUserMasterKey(
								SCAdminShowAssetDetailsScreen.this)));
				listParams.add(new BasicNameValuePair("company_id", ""
						+ SCPreferences.getPreferences().getCompanyId(
								SCAdminShowAssetDetailsScreen.this)));
